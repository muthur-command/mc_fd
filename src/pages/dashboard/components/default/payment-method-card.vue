...
<template>
  <UiCard>
    <UiCardHeader>
      ...
      <UiCardDescription>Add a new payment method to your account.</UiCardDescription>
    </UiCardHeader>
    <UiCardContent class="grid gap-6">
      <UiRadioGroup v-model="paymentType" class="grid grid-cols-3 gap-4">
        <div>
          <UiRadioGroupItem id="card" value="card" class="peer sr-only" aria-label="Card" />
          <UiLabel
            for="card"
        ...
        <div>
          <UiRadioGroupItem id="paypal" value="paypal" class="peer sr-only" aria-label="Paypal" />
          <UiLabel
            for="paypal"
            class="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-transparent p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary"
          >
        ...
        <div>
          <UiRadioGroupItem id="apple" value="apple" class="peer sr-only" aria-label="Apple" />
          <UiLabel
            for="apple"
            class="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-transparent p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary"
          >
      ...
      <div class="grid grid-cols-3 gap-4">
        <div class="grid gap-2">
          ...
          <UiSelect v-model="month">
            <UiSelectTrigger id="month" class="w-full" aria-label="Month">
              <UiSelectValue placeholder="Month" />
            </UiSelectTrigger>
            <UiSelectContent>
              <UiSelectItem v-for="m in 12" :key="m" :value="String(m)">
        ...
        <div class="grid gap-2">
          ...
          <UiSelect v-model="year">
            <UiSelectTrigger id="year" class="w-full" aria-label="Year">
