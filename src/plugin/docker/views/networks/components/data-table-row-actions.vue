...
import { removeNetworkApi } from '@/plugin/docker/api'

interface Props {
  row: Row<NetworkListResponse>
}

const props = defineProps<Props>()
const network = computed(() => props.row.original)
const { t } = useI18n()
const refresh = inject<() => Promise<void>>('networkListFetch', () => Promise.resolve())
const nonRemovableNames = inject<string[]>('networkNonRemovableNames', [])

const canRemove = computed(() => !nonRemovableNames.includes(network.value.name))
...
async function handleRemove() {
  if (!canRemove.value)
    return
  try {
    await removeNetworkApi(network.value.id)
    toast.success(t('docker.networks.messages.deleteSuccess'))
    removeDialogOpen.value = false
    await refresh()
  }
  catch {
    toast.error(t('docker.networks.messages.deleteFailed'))
  }
}
</script>
